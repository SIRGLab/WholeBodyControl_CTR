...
if __name__ == "__main__":
    ...
    mse_y = []
    mse_z = []
    applied_act = reset_actions
    
    
    # t = torch.linspace(0,100,100)
    # target_tip_position = [0.05*torch.sin(2*torch.pi*t/100.0),
    #                                         0.05*torch.cos(2*torch.pi*t/100.0),
    #                                         0.125+0.0*t/50.0 +0.0*torch.sin(2*torch.pi*t/100.0)]
    ...
    for t in range(100):
        ...
        # target_tip_position = torch.tensor([0.05*torch.sin(torch.tensor([2*torch.pi*t/100.0])),
        #                                     0.05*torch.cos(torch.tensor([2*torch.pi*t/100.0])),
        #                                     0.125+0.0*t/50.0 +0.0*torch.sin(torch.tensor([2*torch.pi*t/100.0]))]).unsqueeze(0).to(device)
        ...
        # applied_act = 0.2*applied_act + 0.8*actions
    
        states_batch = sf.odeStepFull(actions)
        reset_states = states_batch
        reset_actions = actions
        reset_states_down_sample  = downsample_simple(reset_states,sample)[:,:,:3].reshape(batch_size,3*sample)   
        reset_states_down_sample += (torch.randint(-150,150,[batch_size,3*sample])/100000.0).to(device)
        # mse_x.append(torch.sqrt(loss_fn(states_batch[-1,0,0],target_tip_position[0,1])).detach().cpu().numpy())
        # mse_y.append(torch.sqrt(loss_fn(states_batch[-1,0,1],target_tip_position[0,1])).detach().cpu().numpy())
        # mse_z.append(torch.sqrt(loss_fn(states_batch[-1,0,2],target_tip_position[0,2])).detach().cpu().numpy())
    ...
    # print (np.mean(mse_x),np.mean(mse_y),np.mean(mse_z))
    # print (np.std(mse_x),np.std(mse_y),np.std(mse_z))
    plt.show()
    exit()
