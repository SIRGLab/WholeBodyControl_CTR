...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        # target_tip_position = torch.tensor([0.01,0.01,0.0],device=device)*torch.sin(torch.tensor([2*torch.pi*i/100.0],device=device)).unsqueeze(0)
        target_tip_position = torch.tensor([0.03*torch.sin(torch.tensor([2*torch.pi*i/100.0])),
                                            0.03*torch.cos(torch.tensor([2*torch.pi*i/100.0])),
                                            0.1+0.0*torch.sin(torch.tensor([2*torch.pi*i/100.0]))]).unsqueeze(0).to(device)
        
        
        # (torch.randint(-500,500,[batch_size,3])/100000.0).to(device)  # Random targets for tip position
