...
class NeuralODEController(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            nn.Linear(2, 64),  # 3*2 current u, 10*current state,  state variables + 3 target positions
            nn.LeakyReLU(),
            nn.Linear(64, 64),
            nn.LeakyReLU(),
            nn.Linear(64, 64),
            nn.LeakyReLU(),
            nn.Linear(64, 3),  # Outputs 3 action variables
            nn.Tanh()
        )
        
        self.min_action_values = torch.tensor([-0.015, -0.015],device=device)  # replace with your desired mins
        self.max_action_values = torch.tensor([ 0.015, 0.015],device=device)        # replace with your desired maxs
    ...
    def forward(self, t, y):
        ...
        ux = tanh_output[:,0]  # batch_size
        uy = tanh_output[:,1] # batch_size
        
        # ttt = t*torch.ones([batch_size,1]).to(device)
        # data = torch.cat([ttt,y[:,12:]],dim=1)
        
        # Original output from network, range [-1, 1] due to Tanh
        # tanh_output = self.net(data)
        ...
        # # ux = y[:, 12] + tanh_output[:,0]  # batch_size
        # # uy = y[:, 13] + tanh_output[:,1] # batch_size
        # ux = scaled_output[:,0]  # batch_size
        # uy = scaled_output[:,1] # batch_size
...
if __name__ == "__main__":
    ...
    # shape_Node.load_state_dict(torch.load("trainedModel/shape_node_20240911-135108.zip")) # residual
    shape_Node.load_state_dict(torch.load("trainedModel/shape_node_20240911-143715.zip")) # u = f(x)
    # shape_Node.load_state_dict(torch.load("trainedModel/shape_node_20240911-151216.zip")) # u = f(t,x)
    # shape_Node.load_state_dict(torch.load("trainedModel/shape_node_temp_6000_20240911-153150.zip")) # u = f(t,x)
    # shape_Node.load_state_dict(torch.load("trainedModel/shape_node_temp_3000_20240911-155430.zip")) # u = f(t,x) range
    ...
    for t in range(1):
        ...
        actions = (torch.randint(-80,80,[batch_size,3*n_seg])/10000.0).to(device)  # Random targets for tip position
        # actions[:,2] = 0.05 
        ...
        y0_batch = sf.y0.unsqueeze(0).repeat(batch_size, 1).to(device)  # (batch_size, 14)
    ...
    for epoch in range(num_epochs):
        ...
        y0_batch = y0.unsqueeze(0).repeat(batch_size, 1).to(device)  # (batch_size, 14)
        y0_batch[:, 12] = ((ux.reshape(1,batch_size) + (torch.randint(-1500,1500,[1,batch_size])/100.0).to(device)))
        y0_batch[:, 13] = ((uy.reshape(1,batch_size) + (torch.randint(-1500,1500,[1,batch_size])/100.0).to(device)))
        
        sol_batch = odeint(shape_Node, y0_batch, t_eval)[:,:,:3]  # (timesteps, batch_size, 14)
        ...
        loss = 1000*loss_fn(sol_masked, states_batch) #+ 2000*loss_fn(sol_masked[:,:,2], states_batch[:,:,2])
