...
class NeuralODEController(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(3*2+3*10+3, 256),  # 3*2 current u, 10*current state,  state variables + 3 target positions
            nn.LeakyReLU(),
            nn.Linear(256, 256),
            nn.LeakyReLU(),
            nn.Linear(256, 256),
...
if __name__ == "__main__":
    ...
    # controller.load_state_dict(torch.load("/home/mohammad/SoftRobot_CORL23/trainedModel/model_controller_20240903-111311.zip"))
    # controller.load_state_dict(torch.load("/home/mohammad/SoftRobot_CORL23/trainedModel/model_controller_temp_5000_20240903-125531.zip"))
    # controller.load_state_dict(torch.load("/home/mohammad/SoftRobot_CORL23/trainedModel/model_controller_temp_8000_20240903-172900.zip"))
    controller.load_state_dict(torch.load("/home/mohammad/SoftRobot_CORL23/trainedModel/model_controller_temp_5000_20240903-170304.zip"))
    ...
    ax.view_init(elev=43, azim=125)  # High elevation, rotated 30 degrees

    batch_size = 1
    reset_actions = (0*torch.randint(-150,150,[batch_size,6])/10000.0).to(device)  # Random targets for tip position
    ...
    for t in range(100):
        target_tip_position = torch.tensor([0.025*torch.sin(torch.tensor([2*torch.pi*t/100.0])),
                                            # 0.04*torch.cos(torch.tensor([2*torch.pi*i/200.0])),
                                            0.08*torch.cos(torch.tensor([2*torch.pi*t/100.0])),
                                            0.07+0.0*t/50.0 +0.0*torch.sin(torch.tensor([2*torch.pi*t/100.0]))]).unsqueeze(0).to(device)
        
        
        T  = 25
        tt = (t*1.0)
        scale = 4.5
        x0 = np.array([-0.0,0.01,0.08])
        ...
        if t % 5 == 0 :   
            for t in range(states_batch.size(1)):
                ...
                ax.plot(states[:, 0], states[:, 1], states[:, 2],linewidth = 1.2,color=(0.0, 0.0, 0.5, 0.4))
    ...
    for epoch in range(num_epochs):
        ...
        states_batch = sf.odeStepFull(actions)
        down_sample = 10
        states_down_sample  = downsample_simple(states_batch,down_sample)[:,:,:3].reshape(batch_size,3*down_sample)   

       
        # Calculate loss using only the tip position (final state position)
        final_tip_position = states_batch[-1, :, :3]  # (batch_size, 3)
        loss = 5000*loss_fn(final_tip_position, target_tip_position) + 100*loss_fn(reset_actions, actions) + 200*loss_fn(states_down_sample, reset_states_down_sample)
